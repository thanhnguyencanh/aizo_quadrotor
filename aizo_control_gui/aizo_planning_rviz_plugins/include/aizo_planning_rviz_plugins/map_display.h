...
#include <aizo_planning_rviz_plugins/data_ros_utils.h>
#include <aizo_planning_rviz_plugins/map_util.h>
...
namespace kr {
class MapDisplay
    : public rviz::MessageFilterDisplay<aizo_planning_msgs::VoxelMap> {
  Q_OBJECT
 public:
  MapDisplay();
  ~MapDisplay();
 ...
 protected:
  void setMap(std::shared_ptr<VoxelMapUtil>& map_util,
              const aizo_planning_msgs::VoxelMap& msg);
  void getMap(std::shared_ptr<VoxelMapUtil>& map_util,
              aizo_planning_msgs::VoxelMap& map);

  void onInitialize();

  void processMessage(const aizo_planning_msgs::VoxelMapConstPtr& map);
