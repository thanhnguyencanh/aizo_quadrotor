...
class SplineTrajectoryVisual {
 public:
  ...
  void setMessage(const aizo_planning_msgs::SplineTrajectory& msg);
  ...
  std::vector<Eigen::VectorXd> sample(
      const aizo_planning_msgs::SplineTrajectory& msg, int N, int deriv_num);
  static Eigen::VectorXd evaluate(const aizo_planning_msgs::SplineTrajectory& msg,
                           double t,
                           uint deriv_num);
