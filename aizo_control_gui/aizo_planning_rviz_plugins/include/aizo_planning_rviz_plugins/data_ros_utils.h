...
#include <aizo_planning_msgs/PathArray.h>
#include <aizo_planning_msgs/Trajectory.h>
#include <aizo_planning_rviz_plugins/data_type.h>
...
vec_Vec3f ros_to_path(const aizo_planning_msgs::Path& msg);

template <int Dim>
aizo_planning_msgs::Path path_to_ros(const vec_Vecf<Dim>& path, double h = 0) {
  aizo_planning_msgs::Path msg;
...
template <int Dim>
aizo_planning_msgs::PathArray path_array_to_ros(const vec_E<vec_Vecf<Dim>>& paths,
                                              double h = 0) {
  aizo_planning_msgs::PathArray msg;
  for (const auto& it : paths) msg.paths.push_back(path_to_ros(it, h));
  return msg;
}

template <int Dim>
aizo_planning_msgs::PathArray path_array_to_ros(
    const std::vector<std::pair<std::string, vec_Vecf<Dim>>>& paths,
    double h = 0) {
  aizo_planning_msgs::PathArray msg;
  for (const auto& it : paths) {
    aizo_planning_msgs::Path path_msg = path_to_ros(it.second, h);
...
Eigen::VectorXd evaluate(const aizo_planning_msgs::Trajectory& msg,
                         double t,
                         int deriv_num);

std::vector<Eigen::VectorXd> sample(const aizo_planning_msgs::Trajectory& msg,
                                    int N,
                                    int deriv_num);
