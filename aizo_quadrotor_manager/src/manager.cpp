...
#include <aizo_quadrotor_manager/manager.h>
...
namespace aizo_quadrotor_manager
{
// Strings
static const std::string line_tracker_distance("aizo_trackers/LineTrackerDistance");
static const std::string line_tracker_min_jerk("aizo_trackers/LineTrackerMinJerk");
static const std::string velocity_tracker_str("aizo_trackers/VelocityTracker");
static const std::string null_tracker_str("aizo_trackers/NullTracker");
static const std::string circle_tracker_str("aizo_trackers/CircleTracker");
static const std::string lissajous_tracker_str("aizo_trackers/LissajousTracker");
static const std::string lissajous_adder_str("aizo_trackers/LissajousAdder");
      ...
      lissajous_adder_client_(nh_, "trackers_manager/lissajous_adder/LissajousAdder", true)
{
  ...
  pub_so3_command_ = nh_.advertise<aizo_planning_msgs::SO3Command>("so3_cmd", 10);
  pub_trpy_command_ = nh_.advertise<aizo_planning_msgs::TRPYCommand>("trpy_cmd", 10);
  pub_position_command_ = nh_.advertise<aizo_planning_msgs::PositionCommand>("position_cmd", 10);
  pub_status_ = priv_nh_.advertise<std_msgs::UInt8>("status", 10);
  pub_goal_velocity_ = nh_.advertise<aizo_tracker_msgs::VelocityGoal>("trackers_manager/velocity_tracker/goal", 10);
  ...
  // Services
  srv_transition_ = nh_.serviceClient<aizo_tracker_msgs::Transition>("trackers_manager/transition");
...
void MAVManager::tracker_done_callback(const actionlib::SimpleClientGoalState &state,
                                       const aizo_tracker_msgs::LineTrackerResultConstPtr &result)
{
...
void MAVManager::circle_tracker_done_callback(const actionlib::SimpleClientGoalState &state,
                                              const aizo_tracker_msgs::CircleTrackerResultConstPtr &result)
{
...
void MAVManager::lissajous_tracker_done_callback(const actionlib::SimpleClientGoalState &state,
                                                 const aizo_tracker_msgs::LissajousTrackerResultConstPtr &result)
{
...
void MAVManager::lissajous_adder_done_callback(const actionlib::SimpleClientGoalState &state,
                                               const aizo_tracker_msgs::LissajousAdderResultConstPtr &result)
{
  ROS_INFO("Lissajous tracking completed. Duration: %2.2f seconds, distance: %2.2f m, now located at (%2.2f, %2.2f, "
           "%2.2f, %2.2f).",
           result->duration, result->length, result->x, result->y, result->z, result->yaw);
...
bool MAVManager::takeoff()
{
  ...
  aizo_tracker_msgs::LineTrackerGoal goal;
...
bool MAVManager::circle(float Ax, float Ay, float T, float duration)
{
  ...
  aizo_tracker_msgs::CircleTrackerGoal goal;
  goal.Ax = Ax;
  goal.Ay = Ay;
  goal.T = T;
  goal.duration = duration;
                           ...
                           float num_cycles, float ramp_time)
{
  ...
  aizo_tracker_msgs::LissajousTrackerGoal goal;
  goal.x_amp = x_amp;
  goal.y_amp = y_amp;
  goal.z_amp = z_amp;
                                    ...
                                    float yaw_num_periods[2], float period[2], float num_cycles[2], float ramp_time[2])
{
  ...
  aizo_tracker_msgs::LissajousAdderGoal goal;
...
bool MAVManager::setDesVelInWorldFrame(float x, float y, float z, float yaw, bool use_position_feedback)
{
  ...
  aizo_tracker_msgs::VelocityGoal goal;
...
bool MAVManager::setPositionCommand(const aizo_planning_msgs::PositionCommand &msg)
{
...
bool MAVManager::setSO3Command(const aizo_planning_msgs::SO3Command &msg)
{
...
bool MAVManager::setTRPYCommand(const aizo_planning_msgs::TRPYCommand &msg)
{
  // Note: To enable motors, the motors method must be used
  if(!this->motors())
  {
...
void MAVManager::output_data_cb(const aizo_planning_msgs::OutputData::ConstPtr &msg)
{
...
void MAVManager::tracker_status_cb(const aizo_tracker_msgs::TrackerStatus::ConstPtr &msg)
{
...
bool MAVManager::eland()
{
  ...
  if(this->motors() && (status_ == FLYING || status_ == ELAND))
  {
    ...
    aizo_planning_msgs::PositionCommand goal;
...
{
  // usleep(100000);
  aizo_tracker_msgs::Transition transition_cmd;
