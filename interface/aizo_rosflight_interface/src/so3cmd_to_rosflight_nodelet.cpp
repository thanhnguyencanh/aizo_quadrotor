...
class SO3CmdToRosflight : public nodelet::Nodelet
{
 ...
 private:
  ...
  aizo_planning_msgs::SO3Command last_so3_cmd_;
};
...
void SO3CmdToRosflight::imu_callback(const sensor_msgs::Imu::ConstPtr &pose)
{
  ...
  if(so3_cmd_set_ && ((ros::Time::now() - last_so3_cmd_time_).toSec() >= so3_cmd_timeout_))
  {
    ...
    const auto last_so3_cmd_ptr = boost::make_shared<aizo_planning_msgs::SO3Command>(last_so3_cmd_);

    so3_cmd_callback(last_so3_cmd_ptr);
  }
}

void SO3CmdToRosflight::so3_cmd_callback(const aizo_planning_msgs::SO3Command::ConstPtr &msg)
{
  if(!so3_cmd_set_)
    so3_cmd_set_ = true;
  ...
  double force = f_des(0) * R_cur(0, 2) + f_des(1) * R_cur(1, 2) + f_des(2) * R_cur(2, 2);
...
void SO3CmdToRosflight::onInit(void)
{
  ...
  so3_cmd_sub_ =
      priv_nh.subscribe("so3_cmd", 10, &SO3CmdToRosflight::so3_cmd_callback, this, ros::TransportHints().tcpNoDelay());

  odom_sub_ =
      priv_nh.subscribe("odom", 10, &SO3CmdToRosflight::odom_callback, this, ros::TransportHints().tcpNoDelay());

  imu_sub_ = priv_nh.subscribe("imu", 10, &SO3CmdToRosflight::imu_callback, this, ros::TransportHints().tcpNoDelay());
}
