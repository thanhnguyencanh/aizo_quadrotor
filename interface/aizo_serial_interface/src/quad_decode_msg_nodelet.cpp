...
#include <aizo_serial_interface/decode_msgs.h>
...
class QuadDecodeMsg : public nodelet::Nodelet
{
 ...
 private:
  void serial_callback(const aizo_planning_msgs::Serial::ConstPtr &msg);
  ros::Publisher output_data_pub_, imu_output_pub_, status_pub_;
  ros::Subscriber serial_sub_;
};

void QuadDecodeMsg::serial_callback(const aizo_planning_msgs::Serial::ConstPtr &msg)
{
  if(msg->type == aizo_planning_msgs::Serial::OUTPUT_DATA)
  {
    aizo_planning_msgs::OutputData::Ptr output_msg(new aizo_planning_msgs::OutputData);
    sensor_msgs::Imu::Ptr imu_msg(new sensor_msgs::Imu);

    if(aizo_planning_msgs::decodeOutputData(msg->data, *output_msg))
    {
  ...
  else if(msg->type == aizo_planning_msgs::Serial::STATUS_DATA)
  {
    aizo_planning_msgs::StatusData::Ptr status_msg(new aizo_planning_msgs::StatusData);
    if(aizo_planning_msgs::decodeStatusData(msg->data, *status_msg))
    {
...
void QuadDecodeMsg::onInit(void)
{
  ...
  output_data_pub_ = priv_nh.advertise<aizo_planning_msgs::OutputData>("output_data", 10);
  imu_output_pub_ = priv_nh.advertise<sensor_msgs::Imu>("imu", 10);
  status_pub_ = priv_nh.advertise<aizo_planning_msgs::StatusData>("status", 10);
