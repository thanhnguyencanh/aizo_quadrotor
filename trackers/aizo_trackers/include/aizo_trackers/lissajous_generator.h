...
class LissajousGenerator
{
 public:
  ...
  void setParams(const aizo_tracker_msgs::LissajousTrackerGoal::ConstPtr &msg);
  void setParams(const aizo_tracker_msgs::LissajousAdderGoal::ConstPtr &msg, int num);
  void generatePath(nav_msgs::Path &path, geometry_msgs::Point &initial_pt, double dt);
  const aizo_planning_msgs::PositionCommand::Ptr getPositionCmd(void);
  bool activate(void);
