...
class VelocityTracker : public aizo_trackers_manager::Tracker
{
 public:
 ...
 private:
  void velocity_cmd_cb(const aizo_tracker_msgs::VelocityGoal::ConstPtr &msg);

  ros::Subscriber sub_vel_cmd_, sub_position_vel_cmd_;
  aizo_planning_msgs::PositionCommand position_cmd_;
...
bool VelocityTracker::Activate(const aizo_planning_msgs::PositionCommand::ConstPtr &cmd)
{
  if(cmd)
  {
...
aizo_planning_msgs::PositionCommand::ConstPtr VelocityTracker::update(const nav_msgs::Odometry::ConstPtr &msg)
{
  ...
  if((ros::Time::now() - last_cmd_time_).toSec() > timeout_)
  {
    ...
    if(use_position_gains_)
      position_cmd_.use_msg_gains_flags = aizo_planning_msgs::PositionCommand::USE_MSG_GAINS_NONE;

    position_cmd_.header.stamp = msg->header.stamp;
    position_cmd_.header.frame_id = msg->header.frame_id;
    last_t_ = 0;
    return aizo_planning_msgs::PositionCommand::ConstPtr(new aizo_planning_msgs::PositionCommand(position_cmd_));
  }
  ...
  else
  {
    ...
    position_cmd_.use_msg_gains_flags = aizo_planning_msgs::PositionCommand::USE_MSG_GAINS_POSITION_ALL;
...
void VelocityTracker::velocity_cmd_cb(const aizo_tracker_msgs::VelocityGoal::ConstPtr &msg)
{
...
{
  return active_ ? static_cast<uint8_t>(aizo_tracker_msgs::TrackerStatus::ACTIVE) :
                   static_cast<uint8_t>(aizo_tracker_msgs::TrackerStatus::SUCCEEDED);
}

#include <pluginlib/class_list_macros.h>
PLUGINLIB_EXPORT_CLASS(VelocityTracker, aizo_trackers_manager::Tracker)
