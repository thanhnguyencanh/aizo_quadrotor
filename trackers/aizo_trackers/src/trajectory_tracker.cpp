...
#include <aizo_tracker_msgs/TrajectoryTrackerAction.h>
#include <aizo_trackers/initial_conditions.h>
#include <aizo_trackers/traj_gen.h>
#include <aizo_trackers_manager/Tracker.h>

class TrajectoryTracker : public aizo_trackers_manager::Tracker
{
 public:
 ...
 private:
  ...
  typedef actionlib::SimpleActionServer<aizo_tracker_msgs::TrajectoryTrackerAction> ServerType;
  ...
  aizo_tracker_msgs::TrajectoryTrackerGoal goal_;
...
bool TrajectoryTracker::Activate(const aizo_planning_msgs::PositionCommand::ConstPtr &cmd)
{
...
aizo_planning_msgs::PositionCommand::ConstPtr TrajectoryTracker::update(const nav_msgs::Odometry::ConstPtr &msg)
{
  pos_set_ = true;
  ...
  auto cmd = boost::make_shared<aizo_planning_msgs::PositionCommand>();
  ...
  if(traj_time >= traj_total_time_)  // Reached goal
  {
    ...
    aizo_tracker_msgs::TrajectoryTrackerResult result;
  ...
  if(!goal_reached_)
  {
    aizo_tracker_msgs::TrajectoryTrackerFeedback feedback;
...
PLUGINLIB_EXPORT_CLASS(TrajectoryTracker, aizo_trackers_manager::Tracker);
