...
class TrackersManager : public nodelet::Nodelet
{
 ...
 private:
  ...
  bool transition_callback(aizo_tracker_msgs::Transition::Request &req, aizo_tracker_msgs::Transition::Response &res);

  ros::Subscriber sub_odom_;
  ros::Publisher pub_cmd_, pub_status_;
  ros::ServiceServer srv_tracker_;
  pluginlib::ClassLoader<aizo_trackers_manager::Tracker> tracker_loader_;
  aizo_trackers_manager::Tracker *active_tracker_;
  std::map<std::string, aizo_trackers_manager::Tracker *> tracker_map_;
  aizo_planning_msgs::PositionCommand::ConstPtr cmd_;
};

TrackersManager::TrackersManager(void)
    : tracker_loader_("aizo_trackers_manager", "aizo_trackers_manager::Tracker"), active_tracker_(NULL)
{
}

TrackersManager::~TrackersManager(void)
{
  for(std::map<std::string, aizo_trackers_manager::Tracker *>::iterator it = tracker_map_.begin();
...
void TrackersManager::onInit(void)
{
  ...
  for(int i = 0; i < tracker_list.size(); i++)
  {
    ...
    {
#if ROS_VERSION_MINIMUM(1, 8, 0)
      aizo_trackers_manager::Tracker *c = tracker_loader_.createUnmanagedInstance(tracker_name);
#else
      aizo_trackers_manager::Tracker *c = tracker_loader_.createClassInstance(tracker_name);
#endif
  ...
  pub_cmd_ = priv_nh.advertise<aizo_planning_msgs::PositionCommand>("cmd", 10);
  pub_status_ = priv_nh.advertise<aizo_tracker_msgs::TrackerStatus>("status", 10);
...
{
  std::map<std::string, aizo_trackers_manager::Tracker *>::iterator it;
  for(it = tracker_map_.begin(); it != tracker_map_.end(); it++)
  {
    if(it->second == active_tracker_)
    {
      ...
      aizo_tracker_msgs::TrackerStatus::Ptr status_msg(new aizo_tracker_msgs::TrackerStatus);
...
bool TrackersManager::transition_callback(aizo_tracker_msgs::Transition::Request &req,
                                          aizo_tracker_msgs::Transition::Response &res)
{
  const std::map<std::string, aizo_trackers_manager::Tracker *>::iterator it = tracker_map_.find(req.tracker);
