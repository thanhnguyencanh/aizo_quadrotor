...
sensor_msgs::PointCloud transform(const sensor_msgs::PointCloud &cloud, 
    ...
    for (size_t i = 0; i < cloud.points.size(); i++) {
        Eigen::Vector3d raw(cloud.points[i].x, cloud.points[i].y, cloud.points[i].z);
...
bool getTF(const ros::Time &t, Eigen::Affine3d &TF){
    ...
    pose.position.z = transformStamped.transform.translation.y;
    pose.position.y = transformStamped.transform.translation.z;
...
{
    ros::init(argc, argv, "chang_frame");
    ros::NodeHandle nh("~");
