...
#include <aizo_quadrotor_controller/SO3Config.h>
#include <aizo_quadrotor_controller/SO3Control.h>
#include <aizo_planning_msgs/Corrections.h>
...
class SO3ControlNodelet : public nodelet::Nodelet
{
 ...
 private:
  ...
  void position_cmd_callback(const aizo_planning_msgs::PositionCommand::ConstPtr &cmd);
  void odom_callback(const nav_msgs::Odometry::ConstPtr &odom);
  void enable_motors_callback(const std_msgs::Bool::ConstPtr &msg);
  void corrections_callback(const aizo_planning_msgs::Corrections::ConstPtr &msg);
  void cfg_callback(aizo_quadrotor_controller::SO3Config &config, uint32_t level);
  ...
  typedef dynamic_reconfigure::Server<aizo_quadrotor_controller::SO3Config> ReconfigureServer;
...
void SO3ControlNodelet::publishSO3Command()
{
  ...
  aizo_planning_msgs::SO3Command::Ptr so3_command = boost::make_shared<aizo_planning_msgs::SO3Command>();
...
void SO3ControlNodelet::position_cmd_callback(const aizo_planning_msgs::PositionCommand::ConstPtr &cmd)
{
...
void SO3ControlNodelet::corrections_callback(const aizo_planning_msgs::Corrections::ConstPtr &msg)
{
...
void SO3ControlNodelet::cfg_callback(aizo_quadrotor_controller::SO3Config &config, uint32_t level)
{
  if(level == 0)
  {
  ...
  NODELET_WARN_STREAM_COND(level != std::numeric_limits<uint32_t>::max() && (level >= (1 << 5)),
                           "aizo_quadrotor_controller dynamic reconfigure called, but with unknown level: " << level);
}
...
void SO3ControlNodelet::onInit(void)
{
  ...
  aizo_quadrotor_controller::SO3Config config;
  ...
  so3_command_pub_ = priv_nh.advertise<aizo_planning_msgs::SO3Command>("so3_cmd", 10);
